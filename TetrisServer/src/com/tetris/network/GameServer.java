...
class GameHandler extends Thread{
	...
	public GameHandler(Socket socket,ArrayList<GameHandler> list, int index, ArrayList<Integer> indexList){
		...
		try{
			DataShip data = (DataShip)ois.readObject(); //ois에서 data를 가져온다. 
			ip = data.getIp();
			name = data.getName();
...
//TODO:--------------------------[ 요청 대기 ]-------------------------
	public void run(){
		...
		if(this.index == 1) {	
			DataShip s_data = new DataShip(DataShip.MASTER_PLAYER);
			send(s_data);
		} else {
			DataShip s_data = new DataShip(DataShip.SLAVE_PLAYER);
			send(s_data);
		}
		...
		while(true){
			...
			if(data.getCommand()==DataShip.CLOSE_NETWORK){
				...
				usingItem(data.getSendIndex(),data.getItem());
				System.out.println("클라이언트로부터 요청 : "+data.getSendIndex() +"에게 아이템을 사용해주세요");
			}
			else if(data.getCommand()==DataShip.SEND_BOARD){
				sendBoard(data.getMap());
			}
	...
	//응답하기 : 아이템사용
		public void usingItem(int index, int item){
			...
			data.setItem(item);
			send(data,index-1);
			System.out.println("GameServer의 usingItem 메소드 : 실행");
		}
	//응답하기 : 화면전송
...
//TODO:--------------------------[ 명령 전송 ]--------------------------[완료]
	...
	private void send(DataShip dataShip, int index){
		GameHandler handler;
		if(list.size()>=index){handler = list.get(index);
			if(handler!=null){
				try{
					handler.getOOS().writeObject(dataShip);
					handler.getOOS().flush();
		...
		System.out.println("GameServer의 send 메소드 : 실행, "+index+"에게 데이터 전송합니다.");
	}// 특정 유저에게만 보내기
